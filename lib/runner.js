...
var elmCoverageSrcDir = (args) => {
  return path.join(coverageDir, args.path, "elm-coverage-src");
};
...
var setupTests = (log, args) => () => {
    ...
    return fs
        ...
        .then(function() {
            return fs.mkdirp(elmCoverageSrcDir(args));
        })
        .then(function() {
            return fs.copyFile(
                path.join(__dirname, "..", "kernel-src", "Coverage.elm"),
                path.join(elmCoverageSrcDir(args), "Coverage.elm")
            );
        })
        .then(function() {
...
var allSourcesFromDir = dir => {
    return new Promise(function(resolve, reject) {
        find.file(/\.(elm|js)$/, dir, resolve);
    });
};

var fixOutOfRootPath = path => {
  return path.startsWith('..') 
    ? path.replace(/\.\./g,'__dotdot')
    : path;
}
    ...
    return allSources(args)
        .then(function(files) {
            return Promise.map(
                ...
                function(file) {
                    return fs.copy(file, path.join(coverageDir, fixOutOfRootPath(file)));
                }
            );
        })
        .then(function() {
